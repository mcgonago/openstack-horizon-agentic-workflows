"""Group B: Table Features tests for Images panel."""
...
async def run_tests(page, horizon_url, admin_user, admin_password,
    ...
    try:
        ...
        results.append({
            ...
            "status": "pass",
            "note": "filter functional" if search_input else "no search filter rendered (expected for Python Images panel)",
        })
    ...
    try:
        ...
        has_status = any("status" in h.lower() for h in header_texts)
        status = "pass" if has_name and has_status else "fail"
