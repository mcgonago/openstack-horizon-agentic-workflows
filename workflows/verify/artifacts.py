...
def generate_manual_testing_guide(verify_dir, state, args):
    ...
    recipe_name = recipe.get("_name", "keypairs")
    state["recipe"] = recipe_name

    template_path = verify_dir / "recipes" / recipe_name / "manual_testing_guide_template.md"
    if template_path.exists():
        guide = template_path.read_text()
        now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
        review = state.get("review_number", "unknown")
        guide = guide.replace("{{GENERATED_DATE}}", now)
        guide = guide.replace("{{REVIEW_NUMBER}}", str(review))
        guide = guide.replace("{{TITLE}}", state.get("title", ""))
        guide = guide.replace("{{RECIPE}}", recipe_name)
        global_idx = sum(len(g["tests"]) for g in recipe["groups"])
    else:
        sections = []
