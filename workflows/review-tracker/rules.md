...
### Clickable URLs (Non-negotiable — applies to ALL artifacts)

All URLs and file references in ANY artifact this workflow produces MUST be
rendered as clickable markdown links. This includes tracker documents,
bridge analysis artifacts, final reports, patch manifests, and verify reports.
No exceptions.
...
**File references anywhere in any artifact:**
...
- This applies inside prose text, tables, `**Source:**` fields, `**File:**` fields,
  and `## References` sections — not only in headers
- If a file reference appears in a code block (``` ``` ```) it does not need linking
...
**Suggested responses (for Gerrit copy-paste):**
- Suggested responses inside `> quote blocks` are meant for Gerrit, which does not
  render markdown. File references in suggested responses stay as plain text.
- File references OUTSIDE the quote block (e.g. in the surrounding analysis) MUST
  be clickable links.

**Code blocks showing source code (bridge artifacts and analysis sections):**

Every fenced code block that displays code from a specific file MUST have a
`**Source:**` line immediately before it with a clickable GitHub link to the
file and line range. The link text is the file path with line range; the URL
points to the GitHub source.
...
This applies to:
- Bridge analysis artifacts (code archaeology investigations)
- Deep-dive analysis sections in tracker documents
- Any other artifact section that quotes source code from a file

This does NOT apply to:
- Generic illustrative snippets (e.g., "the correct pattern is..." with no
  specific file attribution)
- Code inside `> quoted` suggested responses (meant for Gerrit copy-paste)
- Inline code spans (`` `foo.bar()` ``) — these are not code blocks

If you show code from a file, the reader must be able to click through to
that exact location in the source. A code block comment like
`# Volume panel pattern (volumes/tables.py)` is NOT a substitute for a
clickable `**Source:**` link — comments inside code blocks are not clickable.

**General rule:**
- Any URL or file path that any artifact generated by this workflow contains must
  be a clickable markdown link — unless it is inside a code block or a suggested
  response quote block intended for Gerrit copy-paste
